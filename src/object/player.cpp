...
float Player::get_speedlimit() const
{
  return m_speedlimit;
}
...
void Player::set_controller(const Controller *controller_)
{
...
void Player::use_scripting_controller(bool use_or_release)
{
...
void Player::do_scripting_controller(const std::string &control_text, bool pressed)
{
...
bool Player::adjust_height(float new_height, float bottom_offset)
{
...
void Player::trigger_sequence(const std::string &sequence_name, const SequenceData *data)
{
  trigger_sequence(string_to_sequence(sequence_name), data);
}

void Player::trigger_sequence(Sequence seq, const SequenceData *data)
{
  if (m_climbing)
    stop_climbing(*m_climbing);
  stop_backflipping();
...
void Player::update(float dt_sec)
{
  ...
  */
  ...
  if (m_backflipping && !m_dying)
  {
    ...
    if (m_backflip_timer.started())
      m_physic.set_velocity_x(100.0f * static_cast<float>(m_backflip_direction));
...
void Player::handle_input_swimming()
{
...
void Player::swim(float pointx, float pointy, bool boost)
{
  ...
  if (m_swimming && !m_water_jump)
  {
    ...
    if (is_ang_defined && abs(delta) < 0.01f)
      m_swimming_angle = pointed_angle;

    if (!is_ang_defined)
      m_swimming_accel_modifier = 0;
    else
      m_swimming_accel_modifier = 700.f;
  ...
  m_dir = abs(m_swimming_angle) <= math::PI_4 ? Direction::RIGHT : abs(m_swimming_angle) >= 3.f * math::PI_4 ? Direction::LEFT : m_swimming_angle < 0 ? Direction::UP : Direction::DOWN;
...
bool Player::slightly_above_ground() const
{
...
void Player::do_standup(bool force_standup)
{
  if (!m_duck)
    return;
  if (!is_big())
...
void Player::handle_vertical_input()
{
  ...
  if (m_controller->hold(Control::DOWN) && !m_duck && is_big() && !on_ground())
  {
    ...
    if (m_physic.get_velocity_y() >= BUTTJUMP_MIN_VELOCITY_Y)
...
void Player::handle_input()
{
  ...
  if (!m_backflipping && !m_stone && !m_swimming)
  ...
  if (m_controller->pressed(Control::ACTION) && (m_player_status.bonus == FIRE_BONUS || m_player_status.bonus == ICE_BONUS))
  {
         ...
         active_bullets < m_player_status.max_ice_bullets))
    {
      ...
      if (m_physic.get_velocity_x() < 0)
      {
        swim_dir = Direction::LEFT;
      }
      else if (m_physic.get_velocity_x() >= 0)
      {
        swim_dir = Direction::RIGHT;
      }
      if (m_swimming || m_water_jump)
      {
  ...
  if (m_controller->hold(Control::ACTION) && !m_grabbed_object && !m_duck)
  {
    ...
    for (auto &moving_object : Sector::get().get_objects_by_type<MovingObject>())
    {
      ...
      if (portable && portable->is_portable())
      {
        ...
        if (moving_object.get_group() == COLGROUP_DISABLED)
          continue;
...
void Player::handle_input_ghost()
{
  float vx = 0;
  float vy = 0;
...
bool Player::add_bonus(const std::string &bonustype)
{
  return add_bonus(string_to_bonus(bonustype));
}

bool Player::set_bonus(const std::string &bonustype)
{
  return set_bonus(string_to_bonus(bonustype));
}

bool Player::add_bonus(BonusType type, bool animate)
{
...
bool Player::set_bonus(BonusType type, bool animate)
{
  if (m_dying)
  {
    return false;
  ...
  if (type == FIRE_BONUS)
    m_player_status.max_fire_bullets++;
  if (type == ICE_BONUS)
    m_player_status.max_ice_bullets++;
  if (type == AIR_BONUS)
    m_player_status.max_air_time++;
  if (type == EARTH_BONUS)
    m_player_status.max_earth_time++;
...
void Player::set_visible(bool visible_)
{
  m_visible = visible_;
}
...
void Player::draw(DrawingContext &context)
{
  ...
  if (m_swimming && !m_water_jump)
  {
    if (m_dir == Direction::UP && m_physic.get_velocity_x() >= 0)
      sa_postfix = "-up";
    else if (m_dir == Direction::UP && m_physic.get_velocity_x() < 0)
      sa_postfix = "-upflip";
    if (m_dir == Direction::DOWN && m_physic.get_velocity_x() >= 0)
      sa_postfix = "-down";
    else if (m_dir == Direction::DOWN && m_physic.get_velocity_x() < 0)
      sa_postfix = "-downflip";
  }
  //handle waterjump differently to avoid graphical glitch
  if (!m_swimming && m_water_jump)
  {
    if (m_dir == Direction::UP && m_physic.get_velocity_x() > 50)
      sa_postfix = "-up";
    else if (m_dir == Direction::UP && m_physic.get_velocity_x() <= 50)
      sa_postfix = "-upflip";
    if (m_dir == Direction::DOWN && m_physic.get_velocity_x() > 50)
      sa_postfix = "-down";
    else if (m_dir == Direction::DOWN && m_physic.get_velocity_x() <= 50)
      sa_postfix = "-downflip";
  }
  ...
  else if (m_growing)
  {
    if (m_swimming || m_water_jump)
    {
      m_sprite->set_action_continued("swimgrow" + sa_postfix);
    }
    else
    {
...
void Player::kill(bool completely)
{
  ...
  if (!completely && is_big())
  {
    ...
    if (m_player_status.bonus == FIRE_BONUS || m_player_status.bonus == ICE_BONUS || m_player_status.bonus == AIR_BONUS || m_player_status.bonus == EARTH_BONUS)
    {
...
void Player::add_velocity(const Vector &velocity, const Vector &end_speed)
{
  if (end_speed.x > 0)
...
void Player::handle_input_climbing()
{
  if (!m_climbing)
  {
